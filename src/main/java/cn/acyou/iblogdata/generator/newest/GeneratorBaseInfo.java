package cn.acyou.iblogdata.generator.newest;
 ...
 **/
public class GeneratorBaseInfo {
     ...
     */
    private static final String url = "jdbc:mysql://localhost:3306/erp_manager?useInformationSchema=true&useUnicode=true&characterEncoding=UTF-8";
    /**
     * 表名
     */
    private static final String TABLE_NAME = "sys_user_info";
    /**
     * 你的实体类所在的包的位置
     */
    private static final String PACKAGE = "cn.ermei.admui.entity.os";
    /**
     * 类名文件名
     */
    private static final String CLASS_NAME = convertCamelCase("user_info");
    ...
    private static void outputField() {
        String[] targetArray =  ALL_FILED.toString().split(",");
        List<String> stringList = Arrays.stream(targetArray).map(GeneratorBaseInfo::convertcamelCase).map(String::trim).collect(Collectors.toList());
